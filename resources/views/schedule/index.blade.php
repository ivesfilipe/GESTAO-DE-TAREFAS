...
<div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-6">
    <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <form method="POST" action="{{ route('schedule.regenerate') }}">
            @csrf
            <button type="submit" class="rounded-lg border border-slate-300 bg-white dark:bg-white/[0.05] backdrop-blur px-4 py-2.5 text-sm font-medium hover:bg-slate-50 dark:hover:bg-white/[0.04]">
                ↻ Gerar nova sugestão
            </button>
        </form>
    </div>
    ...
    @if($blocks->isEmpty())
        <div class="rounded-xl bg-white dark:bg-white/[0.05] backdrop-blur border border-dashed border-slate-300 dark:border-white/10 p-8 text-center text-sm text-slate-400 dark:text-slate-500">
    ...
    @else
        <form method="POST" action="{{ route('schedule.apply') }}">
            ...
            <div class="rounded-xl bg-white dark:bg-white/[0.05] backdrop-blur border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden mb-5">
                @foreach($blocks as $block)
                    <div class="flex items-center gap-3 px-5 py-3 border-b border-slate-50 last:border-0 {{ $block['overdue'] ? 'bg-red-50/50' : '' }}">
                        <span class="shrink-0 w-32 text-xs font-semibold {{ $block['overdue'] ? 'text-red-600' : 'text-slate-500 dark:text-slate-400' }}">
                            {{ $block['start']->format('d/m') }} · {{ $block['start']->format('H\hi') }}
                        </span>
                        <a href="{{ url('/tarefas/'.$block['task_id']) }}" class="min-w-0 flex-1 truncate text-sm font-medium text-slate-800 dark:text-slate-100 hover:text-brand-600">
                            {{ $block['title'] }}
                            @if($block['overdue'])<span class="ml-1 text-[10px] font-bold uppercase text-red-500">atrasada</span>@endif
                        </a>
                        <span class="shrink-0 text-xs text-slate-400 dark:text-slate-500">{{ $block['start']->diffInMinutes($block['end']) }}min · até {{ $block['end']->format('H\hi') }}</span>
                    </div>
                @endforeach
            </div>
