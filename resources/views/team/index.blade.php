...
<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Equipe</h1>
        <button
            type="button"
    ...
    <div id="invite-form" class="hidden mb-6 rounded-xl bg-white dark:bg-white/[0.05] backdrop-blur border border-slate-200 dark:border-white/10 p-6 shadow-sm">
        <h2 class="text-lg font-semibold text-slate-900 dark:text-white mb-4">Convidar novo liderado</h2>
        <form method="POST" action="{{ route('team.store') }}" class="space-y-4">
            ...
            <div class="flex gap-3">
                ...
                <button type="button" onclick="document.getElementById('invite-form').classList.add('hidden')" class="rounded-lg border border-slate-300 dark:border-white/10 px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/[0.04] transition-colors">
                    Cancelar
                </button>
            </div>
        </form>
    ...
    @if(session('invite_link'))
        <div class="mb-6 rounded-lg bg-green-50 border border-green-200 p-4">
            ...
            <div class="flex items-center gap-2">
                <code class="flex-1 rounded bg-white dark:bg-white/[0.05] backdrop-blur px-3 py-2 text-sm text-slate-700 dark:text-slate-300 border border-green-200 break-all">{{ session('invite_link') }}</code>
                <button
                    type="button"
                    onclick="navigator.clipboard.writeText('{{ session('invite_link') }}')"
                    class="rounded-lg bg-white dark:bg-white/[0.05] backdrop-blur border border-slate-300 dark:border-white/10 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/[0.04]"
                >
                    Copiar
                </button>
            </div>
    ...
    <div class="lg:hidden space-y-3">
        @forelse($liderados as $member)
            <div class="rounded-xl bg-white dark:bg-white/[0.05] backdrop-blur border border-slate-200 dark:border-white/10 p-4 shadow-sm">
                <div class="flex items-center justify-between">
                    ...
                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $member->is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 dark:bg-white/[0.06] text-slate-500 dark:text-slate-400' }}">
                ...
                <div class="mt-3 flex items-center gap-4">
                    ...
                    <form method="POST" action="{{ route('team.destroy', $member) }}" onsubmit="return confirm('Excluir {{ $member->name }}? Esta ação remove o acesso dele(a) ao sistema.');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-sm text-slate-400 dark:text-slate-500 hover:text-red-600 font-medium">
                            Excluir
                        </button>
                    </form>
                </div>
        ...
        @empty
            <div class="rounded-xl bg-white dark:bg-white/[0.05] backdrop-blur border border-slate-200 dark:border-white/10 p-12 text-center shadow-sm">
                ...
                <h3 class="mt-3 text-sm font-medium text-slate-900 dark:text-white">Nenhum liderado</h3>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Convide liderados para começar a delegar tarefas.</p>
            </div>
        @endforelse
    </div>

    <div class="hidden lg:block rounded-xl bg-white dark:bg-white/[0.05] backdrop-blur border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-200 bg-slate-50 dark:bg-white/[0.04]">
                    <th class="px-6 py-3 text-left font-medium text-slate-500 dark:text-slate-400">Nome</th>
                    <th class="px-6 py-3 text-left font-medium text-slate-500 dark:text-slate-400">Email</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                @forelse($liderados as $member)
                    <tr class="hover:bg-slate-50 dark:hover:bg-white/[0.04]/50">
                        <td class="px-6 py-4 font-medium text-slate-900 dark:text-white">{{ $member->name }}</td>
                        <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ $member->email }}</td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $member->is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 dark:bg-white/[0.06] text-slate-500 dark:text-slate-400' }}">
                                {{ $member->is_active ? 'Ativo' : 'Inativo' }}
                            </span>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-4">
                                ...
                                <form method="POST" action="{{ route('team.destroy', $member) }}" onsubmit="return confirm('Excluir {{ $member->name }}? Esta ação remove o acesso dele(a) ao sistema.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-slate-400 dark:text-slate-500 hover:text-red-600 text-sm font-medium">
                                        Excluir
                                    </button>
                                </form>
                            </div>
