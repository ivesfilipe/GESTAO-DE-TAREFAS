...
<div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-6">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500">Perfil inteligente do liderado</p>
        </div>
        <a href="{{ route('team.index') }}" class="rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors text-center">
            Voltar para equipe
        </a>
    </div>
    ...
    <div class="grid lg:grid-cols-3 gap-6 mb-6">
        <div class="rounded-xl bg-white border border-slate-200 p-5 shadow-sm">
            <h2 class="text-sm font-semibold text-slate-700 mb-3">Indicadores operacionais</h2>
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    <span class="text-sm text-slate-500">Tarefas abertas</span>
                    <span class="text-sm font-bold text-slate-800">{{ $metrics['active_tasks'] }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-slate-500">Atrasadas</span>
                    <span class="text-sm font-bold text-red-600">{{ $metrics['overdue_tasks'] }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-slate-500">Concluídas</span>
                    <span class="text-sm font-bold text-green-600">{{ $metrics['completed_tasks'] }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-slate-500">Reprovadas</span>
                    <span class="text-sm font-bold text-orange-600">{{ $metrics['rejected_tasks'] }} ({{ $metrics['rejection_rate'] }}%)</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-slate-500">Ciclo médio</span>
                    <span class="text-sm font-bold text-slate-800">{{ $metrics['avg_cycle_hours'] ? $metrics['avg_cycle_hours'].'h' : 'n/a' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 rounded-xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
                <h2 class="text-sm font-semibold text-slate-700">Perfil profissional</h2>
            ...
            <div id="profile-read" class="p-5 space-y-3">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <p class="text-[11px] uppercase font-semibold text-slate-400">Função</p>
                        <p class="text-sm text-slate-800">{{ $profile?->role ?: '—' }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] uppercase font-semibold text-slate-400">Setor</p>
                        <p class="text-sm text-slate-800">{{ $profile?->department ?: '—' }}</p>
                    </div>
                </div>
                <div>
                    <p class="text-[11px] uppercase font-semibold text-slate-400">Resumo da função</p>
                    <p class="text-sm text-slate-800">{{ $profile?->function_summary ?: '—' }}</p>
                </div>
                <div>
                    <p class="text-[11px] uppercase font-semibold text-slate-400">Responsabilidades</p>
                ...
                <div>
                    <p class="text-[11px] uppercase font-semibold text-slate-400">Responsabilidades recorrentes</p>
                ...
                <div>
                    <p class="text-[11px] uppercase font-semibold text-slate-400">Objetivos profissionais</p>
                ...
                <div>
                    <p class="text-[11px] uppercase font-semibold text-slate-400">Orientações de delegação</p>
                    <p class="text-sm text-slate-800">{{ $profile?->delegation_guidelines ?: '—' }}</p>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('team.profile.update', $user) }}" id="profile-form" class="hidden p-5 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 mb-1">Função</label>
                        <input type="text" name="role" value="{{ old('role', $profile?->role) }}" maxlength="255"
                               class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none"/>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 mb-1">Setor</label>
                        <input type="text" name="department" value="{{ old('department', $profile?->department) }}" maxlength="255"
                               class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none"/>
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-500 mb-1">Resumo da função</label>
                    <textarea name="function_summary" rows="3"
                              class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none">{{ old('function_summary', $profile?->function_summary) }}</textarea>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-500 mb-1">Responsabilidades (uma por linha)</label>
                    <textarea name="responsibilities" rows="3"
                              class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none">{{ old('responsibilities', implode("\n", $profile?->responsibilities ?? [])) }}</textarea>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-500 mb-1">Responsabilidades recorrentes (uma por linha)</label>
                    <textarea name="recurring_responsibilities" rows="3"
                              class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none">{{ old('recurring_responsibilities', implode("\n", $profile?->recurring_responsibilities ?? [])) }}</textarea>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-500 mb-1">Objetivos profissionais (um por linha)</label>
                    <textarea name="professional_objectives" rows="3"
                              class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none">{{ old('professional_objectives', implode("\n", $profile?->professional_objectives ?? [])) }}</textarea>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-500 mb-1">Orientações de delegação</label>
                    <textarea name="delegation_guidelines" rows="3"
                              class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none">{{ old('delegation_guidelines', $profile?->delegation_guidelines) }}</textarea>
                </div>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" id="cancel-profile-edit" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors">
                        Cancelar
                    </button>
    ...
    <div class="grid lg:grid-cols-2 gap-6 mb-6">
        <div class="rounded-xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
                <h2 class="text-sm font-semibold text-slate-700">Análise do perfil</h2>
            ...
            <div id="profile-content" class="p-5">
                @if($profile?->summary)
                    <p class="text-sm text-slate-700 mb-4">{{ $profile->summary }}</p>
                    @if(!empty($profile->strengths))
                        <div class="mb-3">
                            <p class="text-xs uppercase font-semibold text-slate-400 mb-1">Pontos fortes</p>
                            <ul class="list-disc list-inside text-sm text-slate-700 space-y-1">
                    ...
                    @if(!empty($profile->gaps))
                        <div class="mb-3">
                            <p class="text-xs uppercase font-semibold text-slate-400 mb-1">Oportunidades</p>
                            <ul class="list-disc list-inside text-sm text-slate-700 space-y-1">
                    ...
                    @if(!empty($profile->preferences))
                        <div>
                            <p class="text-xs uppercase font-semibold text-slate-400 mb-1">Preferências</p>
                            <ul class="list-disc list-inside text-sm text-slate-700 space-y-1">
                    ...
                    @endif
                @else
                    <p class="text-sm text-slate-400 italic">Clique em "Atualizar inteligência" para gerar uma visão baseada nas tarefas e documentos do liderado.</p>
                @endif
            </div>
            <div id="profile-sources" class="hidden px-5 pb-5">
                <p class="text-xs uppercase font-semibold text-slate-400 mb-1">Fontes</p>
                <ul id="profile-sources-list" class="list-disc list-inside text-xs text-slate-500"></ul>
            </div>
        ...
        <div class="rounded-xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
                <h2 class="text-sm font-semibold text-slate-700">Sugestões de tarefas</h2>
            ...
            <div id="suggestions-content" class="p-5">
                <p class="text-sm text-slate-400 italic">Clique em "Sugerir" para gerar tarefas baseadas exclusivamente nos dados registrados.</p>
            </div>
            <div id="suggestions-sources" class="hidden px-5 pb-5">
                <p class="text-xs uppercase font-semibold text-slate-400 mb-1">Fontes</p>
                <ul id="suggestions-sources-list" class="list-disc list-inside text-xs text-slate-500"></ul>
            </div>
        ...
        <div class="p-5">
            <form method="POST" action="{{ route('team.profile.documents.store', $user) }}" enctype="multipart/form-data" class="mb-5 flex flex-col sm:flex-row gap-3">
                ...
                <input type="file" name="document" accept=".txt,.md,.pdf,.docx" required
                       class="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 file:mr-3 file:rounded file:border-0 file:bg-brand-50 file:px-3 file:py-1 file:text-xs file:font-semibold file:text-brand-700"/>
    ...
    <div class="rounded-xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-3.5 border-b border-slate-100">
            <h2 class="text-sm font-semibold text-slate-700">Comparação de carga</h2>
        </div>
        <div class="p-5">
            <div class="space-y-3">
                @foreach($workload as $member)
                    <div class="flex items-center gap-3">
                        <span class="w-32 text-sm text-slate-600 truncate">{{ $member['name'] }}</span>
                        ...
                        <span class="w-12 text-right text-sm font-medium text-slate-700">{{ $member['active_tasks'] }}</span>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        btn.addEventListener('click', () => {
            ...
            window.axios.post('{{ route('team.profile.summary', $user) }}')
                .then(({ data }) => {
                    ...
                    let html = '';
                    if (p.summary) {
                        html += `<p class="text-sm text-slate-700 mb-4">${p.summary}</p>`;
                    }
                    if (p.strengths && p.strengths.length) {
                        html += `<div class="mb-3"><p class="text-xs uppercase font-semibold text-slate-400 mb-1">Pontos fortes</p><ul class="list-disc list-inside text-sm text-slate-700">${p.strengths.map(s => `<li>${s}</li>`).join('')}</ul></div>`;
                    }
                    if (p.gaps && p.gaps.length) {
                        html += `<div class="mb-3"><p class="text-xs uppercase font-semibold text-slate-400 mb-1">Oportunidades</p><ul class="list-disc list-inside text-sm text-slate-700">${p.gaps.map(g => `<li>${g}</li>`).join('')}</ul></div>`;
                    }
                    if (p.preferences && p.preferences.length) {
                        html += `<div><p class="text-xs uppercase font-semibold text-slate-400 mb-1">Preferências</p><ul class="list-disc list-inside text-sm text-slate-700">${p.preferences.map(p => `<li>${p}</li>`).join('')}</ul></div>`;
                    }
                    content.innerHTML = html || '<p class="text-sm text-slate-600">Análise gerada.</p>';
            ...
            window.axios.post('{{ route('team.profile.suggestions', $user) }}')
                .then(({ data }) => {
                    if (!data.ok || !data.suggestions.length) {
                        content.innerHTML = '<p class="text-sm text-slate-600">Nenhuma sugestão encontrada nos dados registrados.</p>';
                        return;
                    }
                        ...
                        html += `
                            <div class="rounded-lg border border-slate-100 p-3">
                                <div class="flex items-center justify-between gap-2 mb-1">
                                    ...
                                    <span class="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600">${categoryLabels[s.category] || s.category}</span>
                                </div>
                                <p class="text-xs text-slate-500 mb-1">${s.objective}</p>
                                <p class="text-xs text-slate-500 mb-2"><strong>Por que:</strong> ${s.reason}</p>
