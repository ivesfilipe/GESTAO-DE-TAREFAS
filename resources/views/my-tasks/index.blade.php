...
<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
    <div class="mb-6 animate-fade-in-up">
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">Minhas Tarefas</h1>
        <p class="mt-1 text-sm text-slate-500">Foco no agora · priorize o que importa</p>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-orange-50/90 via-orange-50/60 to-amber-50/50 backdrop-blur border border-orange-200/60 p-6 shadow-sm hover-lift animate-entrance stagger-1">
            <div class="pointer-events-none absolute -top-12 -right-12 size-32 rounded-full bg-orange-200/30 blur-2xl"></div>
            <div class="relative flex items-center justify-between">
                <h2 class="font-semibold tracking-tight text-slate-900 flex items-center gap-2.5">
            ...
            <div class="relative mt-5 space-y-3">
                @forelse($urgentes ?? [] as $idx => $task)
                    <a href="{{ route('tasks.show', $task) }}" class="group block rounded-xl bg-white/90 backdrop-blur border border-orange-200/60 p-4 shadow-sm hover:shadow-md hover:border-orange-300 hover:-translate-y-0.5 transition-all animate-entrance" style="animation-delay: {{ 100 + $idx*70 }}ms">
                        <div class="flex items-start justify-between gap-2">
                            <h3 class="text-sm font-semibold leading-snug text-slate-900 group-hover:text-orange-700 transition-colors">{{ $task->title }}</h3>
                ...
                @empty
                    <div class="rounded-xl border border-dashed border-orange-300/50 bg-white/60 p-6 text-center">
                        <p class="text-sm font-medium text-orange-700">Nenhuma tarefa urgente.</p>
                        <p class="mt-1 text-xs text-orange-600/70">Ótimo — mantenha o foco nas entregas de hoje.</p>
                    </div>
                @endforelse
            </div>
        </div>

        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand-50/90 via-sky-50/50 to-white/80 backdrop-blur border border-brand-200/60 p-6 shadow-sm hover-lift animate-entrance stagger-2">
            ...
            <div class="relative mt-5 space-y-3">
                @forelse($hoje ?? [] as $idx => $task)
                    <a href="{{ route('tasks.show', $task) }}" class="group block rounded-xl bg-white/90 backdrop-blur border border-brand-200/50 p-4 shadow-sm hover:shadow-md hover:border-brand-300 hover:-translate-y-0.5 transition-all animate-entrance" style="animation-delay: {{ 160 + $idx*70 }}ms">
                        <div class="flex items-start justify-between gap-2">
                            <h3 class="text-sm font-semibold leading-snug text-slate-900 group-hover:text-brand-700 transition-colors">{{ $task->title }}</h3>
                            <span class="shrink-0 inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold {{ $priorityBadges[$task->priority] }}">
                                {{ ucfirst($task->priority) }}
                            </span>
                        </div>
                ...
                @empty
                    <div class="rounded-xl border border-dashed border-brand-200/50 bg-white/60 p-6 text-center">
                        <p class="text-sm font-medium text-brand-700">Nada para hoje.</p>
                        <p class="mt-1 text-xs text-brand-600/70">Tudo em dia — aproveite para adiantar próximas.</p>
                    </div>
                @endforelse
            </div>
        </div>

        <div class="relative overflow-hidden rounded-2xl bg-white/80 backdrop-blur border border-slate-200/60 p-6 shadow-sm hover-lift animate-entrance stagger-3">
            ...
            <div class="relative mt-5 space-y-3">
                @forelse($proximas ?? [] as $idx => $task)
                    <a href="{{ route('tasks.show', $task) }}" class="group block rounded-xl bg-white border border-slate-200/70 p-4 shadow-sm hover:shadow-md hover:border-slate-300 hover:-translate-y-0.5 transition-all animate-entrance" style="animation-delay: {{ 220 + $idx*70 }}ms">
                        <div class="flex items-start justify-between gap-2">
                            <h3 class="text-sm font-semibold leading-snug text-slate-900 group-hover:text-brand-700 transition-colors">{{ $task->title }}</h3>
                            <span class="shrink-0 inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold {{ $priorityBadges[$task->priority] }}">
                                {{ ucfirst($task->priority) }}
                ...
                @empty
                    <div class="rounded-xl border border-dashed border-slate-300/60 bg-slate-50/60 p-6 text-center">
                        <p class="text-sm font-medium text-slate-600">Nenhuma tarefa pendente.</p>
                        <p class="mt-1 text-xs text-slate-400">Quando novas tarefas chegarem, aparecem aqui.</p>
                    </div>
                @endforelse
            </div>
        </div>
